...
<div id="dropdown{{ $key }}" class="z-10 hidden bg-white border rounded-lg shadow-md w-44 ">
    <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefaultButton">
        <li>
            <button type="button" onclick="editKategori{{ $key }}.showModal()" class=" px-4 py-2 hover:bg-gray-100 w-full text-Sidebar flex gap-2 items-center">
                <i class="ri-edit-2-fill text-xl"></i>
                Edit Kategori
            </button>
        </li>
        <li>
            <button type="button" onclick="deleteKategori{{ $key }}.showModal()"
