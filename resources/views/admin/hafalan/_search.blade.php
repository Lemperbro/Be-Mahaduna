<form class="w-full" action="{{ route('monitoring.hafalan') }}" method="GET">
    ...
    <div class="flex border border-main3 rounded-lg focus:ring-0 focus:outline-none focus:border-main2 bg-white">
        ...
        <button id="dropdown-button" data-dropdown-toggle="filter"
            ...
            @if (request('tahun') || request('jenjang') || request('bulan'))
            ...
            <ul class="py-2 text-sm text-gray-700 " aria-labelledby="dropdown-button">
                ...
                <li class="flex justify-between items-center px-4 hover:bg-gray-100">
                    <button type="button" onclick="bulanFilter.showModal()"
                        class="inline-flex w-full py-2 {{ request('bulan') !== null ? 'font-semibold' : '' }}">Bulan Monitoring</button>
                    @if (request('bulan') !== null)
                        <i class="ri-checkbox-circle-fill text-lime-500 text-[15px]"></i>
                    @endif
                </li>
...
@include('admin.hafalan.modal-filter._modalTahunFilter')
@include('admin.hafalan.modal-filter._modalJenjangFilter')
@include('admin.hafalan.modal-filter._modalBulanFilter')
