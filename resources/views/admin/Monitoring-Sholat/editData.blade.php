...
@section('container')
    <section>
        ...
        <div class="pt-24 min-h-screen flex flex-col max-w-[1000px] mx-auto justify-center">
            <form action="{{ $type === 'sholat' ? route('monitoring.update', ['type' => 'sholat', 'id' => $data->monitor_mingguan_id]) : route('monitoring.update', ['type' => 'ngaji', 'id' => $data->monitor_mingguan_id]) }}" method="POST" class="w-full mx-auto">
                @csrf
                <div>
                    <label>Santri</label>
                    <div class="mt-1">
                        <input type="text" class="w-full p-2 rounded-md  border-main3 focus:ring-0 focus:outline-none focus:border-main2 bg-gray-200 text-gray-700" value="{{ $data->santri->nama }}" readonly>
                    </div>
                </div>
                <div class="mt-4">
