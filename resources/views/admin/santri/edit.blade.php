...
@section('container')
    <section>
        ...
        <div class="min-h-screen pt-24">
            <form action="{{ route('santri.update', ['id' => $data->santri_id]) }}" method="POST" class="max-w-[800px] mx-auto mt-10">
                @csrf
                <div>
                    ...
                    peer
                @enderror"
                        placeholder="Masukan Nama Santri" value="{{ $data->nama }}">
                    @error('nama')
                ...
                <div class="mt-4">
                    ...
                    peer
                @enderror"
                        placeholder="Masukan Nama Santri" value="{{ $data->tgl_lahir }}">
                    @error('tgl_lahir')
                        <p class="peer-invalid:visible text-red-700 font-light">
                            {{ $message }}
                        </p>
                    ...
                    peer @enderror">
                        @foreach ($jenjang as $item)
                            <option value="{{ $item->jenjang_id }}" class="capitalize"
                                {{ $data->jenjang_id == $item->jenjang_id ? 'selected' : '' }}>{{ ucwords($item->jenjang) }}
                            </option>
                        @endforeach
                    </select>
                    @error('jenjang')
                    ...
                    <div class="mt-1">
                        ...
                        @enderror">
                            @foreach ($wali as $item)
                                <option value="{{ $item->wali_id }}" class="flex justify-between" {{ $data->waliRelasi->wali_id == $item->wali_id ? 'selected' : '' }}>
